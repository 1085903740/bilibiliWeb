<template>
  <div>
    <navbar/>
    <van-divider dashed :style="{color:'#1a73e8',borderColor:'1a73e8'}">-------------------------现有栏目-------------------------</van-divider>
      <div class="new-category newcategory">
        <p @click="handleCategoryClick(index)" v-for="(item,index) in newCat" :key="index">
          {{item.title}}
        </p>
      </div>
    <van-divider dashed :style="{color:'#1a73e8',borderColor:'1a73e8'}">-------------------------删除栏目-------------------------</van-divider>
      <div class="new-category delcategory">
        <p @click="deleteCategoryClick(index)" v-for="(item,index) in delCat" :key="index">
          {{item.title}}
        </p>
      </div>
  </div>
